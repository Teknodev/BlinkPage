...
const assertConversionBadgeVisible = (index: number) => {
  cy.get(`[data-cy="interaction-item-${index}"]`, { timeout: 5000 })
    .should('contain.text', 'Conversion Goal');
};
...
const assertConversionBadgeAbsent = (index: number) => {
  cy.get(`[data-cy="interaction-item-${index}"]`, { timeout: 5000 })
    .should('not.contain.text', 'Conversion Goal');
};
...
describe('Conversion Goal — Visibility', () => {
  ...
  it('should show the Conversion Goal section when trigger is click (page-load default)', () => {
...
describe('Conversion Goal — Toggle ON', () => {
  ...
  it('should mark interaction as conversion goal when switch is toggled ON', () => {
    openInteractionsPanel();
    addInteractionAndOpenPopup();
