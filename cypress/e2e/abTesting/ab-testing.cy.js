...
describe('A/B Testing — Panel Navigation', () => {
  ...
  it('should display A/B Testing heading in the settings modal', () => {
    abTestingPage.openAbTestingPanel();
    cy.get('[data-cy="ab-testing-section"]').contains('A/B Testing').should('exist');
  });
});
...
describe('A/B Testing — Flow Builder (requires Pro entitlement stub)', () => {
  ...
  it('should render the flow builder canvas when AB testing is enabled', () => {
      ...
      if (createBtn.length) {
        ...
        cy.get('body').then(($b) => {
          if ($b.find('[data-cy="ab-test-name-input"], input[placeholder*="name" i]').length) {
            ...
            cy.get('body').find('button').contains(/create|confirm/i).first().click({ force: true });
          }
        });
