 ...
 *   - The popover's per-language row in `LocalizationManager.tsx` lacks a
 *     `data-cy="popover-lang-row-{code}"` attribute. We currently use
 *     `cy.contains` against the visible language name.
 *
 * Hard constraints:
 ...
 * each row contains a LanguageSelect with the language's localized name.
 * We click the row by visible language-name match.
 */
const switchPopoverToLanguage = (langName) => {
  ...
  cy.get('[data-cy="localization-edit-btn"]')
    ...
    .last()
    .within(() => {
      cy.contains(langName, { timeout: 10000 }).click({ force: true });
    });
};
...
describe('Page.updateComponent — language-scoped writes (Regression)', () => {
  ...
  it('canvas inline-edit in EN does NOT overwrite TR copy of the same prop', () => {
    ...
    // 4) Re-open the localization-settings overlay; assert EN cell now shows
    //    the edited value AND TR cell still shows TR_VALUE (untouched).
    openPopover();
    openSettingsOverlay();
    cy.get('[data-cy="localization-settings-page"]')
      .find('[role="gridcell"]')
      .eq(2)
      .should('contain.text', EN_EDIT);
    cy.get('[data-cy="localization-settings-page"]')
      .find('[role="gridcell"]')
      .eq(3)
      .should('contain.text', TR_VALUE);
    closeSettingsOverlay();
  });
  ...
  it('content-tab edit in EN does NOT overwrite TR copy of the same prop', () => {
    ...
    // 4) Verify TR cell remained TR_VALUE.
    openPopover();
    openSettingsOverlay();
    cy.get('[data-cy="localization-settings-page"]')
      .find('[role="gridcell"]')
