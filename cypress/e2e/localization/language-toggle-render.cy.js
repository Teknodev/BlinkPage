...
const resetProjectLanguagesViaApi = () => {
  cy.window({ log: false }).then((win) => {
    ...
    cy.request({
      ...
      cy.log(`[cleanup] languages reset → status ${res.status}`);
    });
  });
};
...
const switchActiveLanguage = (langName) => {
  openPopover();
  // Click the row whose visible text matches the language name. The popover
  // portals to body so we scope by ancestor that has the edit-btn.
  cy.contains(langName, { timeout: 10000 })
    .filter(':visible')
    .first()
    .click({ force: true });
...
describe('Editor.handleLanguageChange — correct content per language (Regression)', () => {
  ...
  it('renders TR content when toggled EN → TR (and re-renders EN on TR → EN)', () => {
    ...
    // 2) Active language is EN by default — playground must show EN_SENTINEL.
    cy.get('[data-cy="playground"]', { timeout: 10000 })
      .should('contain.text', EN_SENTINEL)
      .and('not.contain.text', TR_SENTINEL);

    // 3) Toggle to TR — playground must show TR_SENTINEL only.
    switchActiveLanguage(TR_NAME);
    cy.get('[data-cy="playground"]', { timeout: 10000 })
      .should('contain.text', TR_SENTINEL)
      .and('not.contain.text', EN_SENTINEL);

    // 4) Toggle back to EN — playground must show EN_SENTINEL only.
  ...
  it('handles rapid TR → AB → TR toggling without state leak from prior language', () => {
    ...
    cy.get('[data-cy="playground"]', { timeout: 10000 })
      .should('contain.text', TR_SENTINEL)
      .and('not.contain.text', EN_SENTINEL)
      .and('not.contain.text', AB_SENTINEL);

    // 3) AB
    switchActiveLanguage(AB_NAME);
    cy.get('[data-cy="playground"]', { timeout: 10000 })
      .should('contain.text', AB_SENTINEL)
      .and('not.contain.text', TR_SENTINEL)
      .and('not.contain.text', EN_SENTINEL);

    // 4) TR again — must come back cleanly with TR_SENTINEL, no AB residue.
    switchActiveLanguage(TR_NAME);
    cy.get('[data-cy="playground"]', { timeout: 10000 })
      .should('contain.text', TR_SENTINEL)
      .and('not.contain.text', AB_SENTINEL)
