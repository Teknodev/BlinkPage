...
describe('AI Intent Confirmation Flow', () => {
  ...
  describe('Test 1 — Intent confirmation card UI', () => {
    ...
    it('shows discovery tool rows silently then renders ConfirmationCard with plain-English description only', () => {
      ...
      cy.get('[data-cy="ai-assistant-confirm-card"]').should(
        'contain.text',
        "I'll rewrite your Hero section with a Paris bakery brief."
      );

      // NO technical heading — "Confirm action:" must NOT appear
      cy.get('[data-cy="ai-assistant-confirm-card"]').should(
        'not.contain.text',
        'Confirm action:'
      );
  ...
  describe('Test 2 — Proceed triggers mutation and shows success message', () => {
    ...
    it('clicking Proceed re-submits with confirmed_tool_calls and surfaces the mutation result', () => {
      cy.get('[data-cy="ai-assistant-fab"]').click();
      cy.get('[data-cy="ai-assistant-composer-input"]').type(
        'Change my hero section content to match a bakery in Paris'
      );
      ...
      cy.get('[data-cy="ai-assistant-messages"]').should(
        'contain.text',
        "I've rewritten your Hero section with a Paris bakery theme"
      );
  ...
  describe('Test 3 — Cancel aborts cleanly with no canvas change', () => {
    ...
    it('clicking Cancel removes the card, shows a cancellation note, and fires no second request', () => {
      cy.get('[data-cy="ai-assistant-fab"]').click();
      cy.get('[data-cy="ai-assistant-composer-input"]').type(
        'Change my hero section content to match a bakery in Paris'
      );
  ...
  describe('Test 4 — Explicit approval bypasses the confirmation card', () => {
    ...
    it('a message with explicit approval goes straight to mutation without showing a ConfirmationCard', () => {
      ...
      cy.get('[data-cy="ai-assistant-messages"]').should(
        'contain.text',
        'Updated your About section for a roof repair company in Toronto.'
      );
